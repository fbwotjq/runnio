...
$(document).ready(function() {
	...
	/* bind tab automatic */
	var autoTabInterval;
	$('.bind-tab.auto').each(function() {
		var tabElement = $(this);
		autoTabInterval = setInterval(function() {
			if(!$('li:has(a.active)', tabElement).is(':last-child')) {
				$('li:has(a.active)',tabElement).next('li').find('a').click();
			}else{
				$('li:eq(0)',tabElement).find('a').click();
			}
		}, 5000);
	});
	
	$('.bind-tab.auto').bind('mousedown', function() {
	...
	/* widget tabs */
	$('.bind .bind-cont.active').fadeIn(500);
	
	$('.bind-tab').on('click', 'a', function() {
		
		var parentTab = $(this).closest('.bind-tab');
		var allTabs = $(this).closest('.bind-tab').find('a');
		var thisElement = $(this);
		
		allTabs.removeClass('active');
		thisElement.addClass('active');
		
		parentTab.next().find('.bind-cont').hide();
		parentTab.next().find('.bind-cont[data-box="' + thisElement.data('element') + '"]').fadeIn(500);
